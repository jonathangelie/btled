...
uint8_t gattc_read_req(uint8_t devId, uint8_t *data,
					   uint8_t data_len)
{
	uint8_t ret = BTLE_SUCCESS;
	struct cmd_adaper *adapter;
	...
	if (!ret) {
		...
		if (!bt_gatt_client_read_value(adapter->cli->gatt, handle,
							gattc_read_complete, adapter, NULL)) {
			ret = BTLE_ERROR_INTERNAL;
		}
	}
...
static void cmd_att_disconnect_cb(int err, void *user_data)
{
	struct cmd_adaper *adapter = user_data;
...
static void gattc_disc_desc_complete(struct gatt_db_attribute *attr, void *user_data)
{
	struct cmd_adaper *adapter = user_data;
...
static void gattc_disc_char_complete(struct gatt_db_attribute *attr, void *user_data)
{
	struct cmd_adaper *adapter = user_data;
	bt_uuid_t uuid;
	...
	bt_uuid_to_string((const bt_uuid_t*)&uuid, (char *)msg.uuid_str, sizeof(msg.uuid_str));
...
static void gattc_disc_prim_complete(struct gatt_db_attribute *attr, void *user_data)
{
	struct cmd_adaper *adapter = user_data;
	bool is_primary;
	bt_uuid_t uuid;
...
static void dicovery_done(bool success, uint8_t att_ecode, void *user_data)
{
	...
	gatt_db_foreach_service(adapter->cli->db, NULL, gattc_disc_prim_complete, adapter);
...
static struct client *client_create(struct cmd_adaper *adapter, int fd, uint16_t mtu)
{
	struct client *cli;
	...
	if (!bt_att_register_disconnect(cli->att, cmd_att_disconnect_cb, adapter,
								NULL)) {
	...
	bt_gatt_client_set_debug(cli->gatt, gatt_debug_cb, adapter,
								NULL);


	bt_gatt_client_ready_register(cli->gatt, dicovery_done, adapter, NULL);
	bt_gatt_client_set_service_changed(cli->gatt, gattc_service_changed_cb, adapter,
									NULL);
...
uint8_t gattc_connect(uint8_t devId, uint8_t *data, uint8_t data_len)
{
	...
	if (data_len < 18) {
		...
		if (hci_devba(devId, &src_addr) >= 0) {
			fd = l2cap_le_att_connect(&src_addr, &dst_addr, *dst_addr_type, sec_level);
			if (fd > 0) {
				adapter->devId = devId;
